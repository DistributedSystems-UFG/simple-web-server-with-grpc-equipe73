...
def run():
    with grpc.insecure_channel(const.IP+':'+const.PORT) as channel:
        ...
        # Query an Sensor's data with date
        dt = random.choice(dates)
        response = stub.GetSensorDataFromdate(SensorService_pb2.Sensordate(date = dt))
        print ('Sensor\'s data with date as '+dt+': ' + str(response))

        # Query an Sensor's data with date
        loc = random.choice(locations)
        response = stub.GetSensorDataFromlocation(SensorService_pb2.Sensorlocation(location = random.choice(locations)))
        print ('Sensor\'s data with location as '+loc+': ' + str(response))
